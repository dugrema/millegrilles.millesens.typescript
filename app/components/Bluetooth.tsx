...
import useBluetoothStore from "../utils/bluetooth/store";
...
export function BluetoothAvailableCheck(props: { hide?: boolean }) {
  ...
  const available = useBluetoothStore((state) => state.bluetoothAvailable);
  const setBluetoothAvailable = useBluetoothStore(
    (state) => state.setBluetoothAvailable,
  );
  ...
  useEffect(() => {
    ...
    checkBluetoothAvailable()
      ...
      .catch((err) =>
        console.error("Error checking if Bluetooth is avaiable", err),
      )
      .finally(() => setDone(true));
  }, [available, done, setBluetoothAvailable]);

  if (props.hide) return <></>;

  if (!done) return <div>Checking if Bluetooth is available</div>;
  if (!available) {
    return <div>Bluetooth is not available.</div>;
  }
...
export function DeviceDetail(props: DeviceDetailProps) {
  ...
  const clearDeviceState = useBluetoothStore((state) => state.clear);
